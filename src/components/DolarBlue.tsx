...
import { DolarBlueDatos } from "../services/types/DolarBlueDatos";
import "../css/dolarBlue.css";
import IconoInfo from "./IconoInfo";

const DolarBlue = () => {
  const [dolarBlue, setDolarBlue] = useState<DolarBlueDatos | null>(null);

  useEffect(() => {
    async function obtenerDatosDolarBlue() {
      try {
        const respuesta = await fetch("https://dolarapi.com/v1/dolares/blue");
        const datos: DolarBlueDatos = await respuesta.json();
        setDolarBlue(datos);
      } catch (error) {
  ...
  return (
    <div className="dolarBlue">
      ...
      {dolarBlue ? (
        <div >
        <div className="grupoBlue">
          <p className="compraBlue">
          <strong>Compra: </strong>${dolarBlue.compra}
          </p>
          <p className="ventaBlue">
          <strong>Venta: </strong>${dolarBlue.venta}
        </p>
        </div>
        <p><strong>Diferencia: </strong> ${(dolarBlue.venta - dolarBlue.compra).toLocaleString("es-AR", {
              minimumFractionDigits: 2, 
              maximumFractionDigits: 2,
            })}</p>
         <p className="actualizacionBlue">
         <strong>Ultima actualización: {String.fromCodePoint(0x23f3)} </strong>
